...
function removeProperties(obj, keys) {
  ...
  if (Array.isArray(keys)) {
    ...
    if (Object.prototype.hasOwnProperty.call(newObj, keys)) {
      delete newObj[keys];
    }
  }
  return newObj;
...
function fromJSON(proto, json) {
  ...
  Object.keys(parsedJSON).forEach((prop) => {
    obj[prop] = parsedJSON[prop];
  });
...
class CssSelectorBuilder {
  ...
  checkDuplicate(currentOrder) {
    if (currentOrder === 1 && this.elementCount > 0) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (currentOrder === 2 && this.idCount > 0) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (currentOrder === 6 && this.pseudoElementCount > 0) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
